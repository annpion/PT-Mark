...
from diffusers import StableDiffusionPipeline, StableDiffusionControlNetPipeline, StableDiffusionInpaintPipeline
...
class WatermarkStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    # 注意：当 latents 需要梯度时，使用 gradient checkpointing 来节省内存
    def decode_latents_tensor(self, latents):
        latents = 1 / self.vae.config.scaling_factor * latents

        # 只有当 latents 需要梯度时，VAE decode 才需要梯度
        if latents.requires_grad:
            # 训练模式：使用 gradient checkpointing 减少内存
            # 定义一个简单的 forward 函数用于 checkpoint
            def vae_decode_fn(latents_input):
                return self.vae.decode(latents_input).sample

            # 使用 checkpoint 只在必要时保存中间激活
            image = checkpoint(vae_decode_fn, latents, use_reentrant=False)
        else:
    ...
    def __call__(
        ...
        return_latents_list: bool = False,
        eval = -1
        ...
        prompt_embeds_clone = prompt_embeds.clone()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if text_embeddings is not None:
                    if do_classifier_free_guidance:
                        # if i>eval:
                        #     prompt_embeds = torch.cat([negative_prompt_embeds, prompt_embeds_clone])
                        # else:
                            prompt_embeds = torch.cat([text_embeddings[i].expand(*prompt_embeds_clone.shape), prompt_embeds_clone])
                    else:
                        prompt_embeds = text_embeddings[i]
                ...
                # 当返回 latents_list 时，需要 detach 以避免破坏梯度流
                # 如果 use_trainable_latents=True，我们只保存值，不保存计算图
                if use_trainable_latents:
                    latents_list.append(latents.detach().clone())
                else:
                    latents_list.append(latents.clone() if latents.requires_grad else latents)
        ...
        if return_latents_list:
            return ModifiedStableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept, init_latents=latents_list)
...
class WMDetectStableDiffusionPipeline(WatermarkStableDiffusionPipeline):
    ...
    def backward_diffusion(
        ...
        **kwargs,
        ...
        if return_latents_list is True:
            return latents_list
        return latents


class WMDetectStableDiffusionControlNetPipeline(StableDiffusionControlNetPipeline):
    """ControlNet版本的水印检测Pipeline，继承自StableDiffusionControlNetPipeline"""
    ...
    def __init__(self,
        ...
        super(WMDetectStableDiffusionControlNetPipeline, self).__init__(
            vae=vae,
    ...
    def __call__(
        ...
        """
        ControlNet版本的调用方法，支持水印相关的参数

        Args:
            image: ControlNet的条件图像
        ...
        # 4. Prepare image for ControlNet
        if image is None:
            # 如果没有提供control image，创建一个全零的
            print("Warning: No control image provided for ControlNet, using zero image.")
        ...
        if not use_trainable_latents:
            ...
            if init_latents is None:
                raise ValueError(f"We must have an initial trainable latents.")
            else:
                latents = init_latents
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # predict the noise residual with ControlNet
                ...
                if not use_trainable_latents:
                    ...
                    # 使用checkpoint进行梯度计算
                    def unet_with_controlnet(latent_input, timestep, encoder_hidden_states):
                        ...
                        return self.unet(
                            ...
                            mid_block_additional_residuals=mid_block_res,
                        ).sample

                    noise_pred = checkpoint(unet_with_controlnet, latent_model_input, t, prompt_embeds).sample
        ...
        return ModifiedStableDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept, init_latents=init_latents)
class WMDetectStableDiffusionInpaintPipeline(StableDiffusionInpaintPipeline):
    """Inpainting版本的水印检测Pipeline，支持在inpainting过程中注入和检测水印"""
