...
import scipy
import torch
from ptmark_optim_utils import randn_tensor
# from ptmark_utils import randn_tensor
...
class GTWatermark():
    ...
    def inject_watermark_steal(self, latents, gt_patch, r): 
        latents_fft = torch.fft.fftshift(torch.fft.fft2(latents), dim=(-1, -2))
        # latents_fft[self.watermarking_mask] = self.gt_patch[self.watermarking_mask].clone()
        watermarking_mask = torch.zeros(gt_patch.shape, dtype=torch.bool).to(self.device)
        watermarking_mask[:,self.w_channel] = torch.tensor(self._circle_mask(latents_fft.shape[-1], r=r)).to(self.device)
        latents_fft = latents_fft * ~(watermarking_mask) + gt_patch * watermarking_mask
        latents_w = torch.fft.ifft2(torch.fft.ifftshift(latents_fft, dim=(-1, -2))).real
        return latents_w
